...
class IliasDataPlotter:
    # Diese Klasse wird zum anzeigen der ILIAS Ergebnisse genutzt. Sie berücksichtigt die verschiedenen Ausgabeformate
    # der in ILIAS üblichen Varianten
...
class IliasParser:
    # Eine Klasse zum Einlesen von ILIAS-Daten. Dabei sollen die Testergebnisse anonym zur weiteren
...
class IRT_Plotter:
    # Eine Klasse zur einfacheren Nutzung des IRT-Moduls. Die eingelesenen Daten müssen in dem Format der Bewerte ILIAS-
    # Testergebnisse sein.
    def __init__(self, filename):
