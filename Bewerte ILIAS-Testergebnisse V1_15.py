# -*- coding: utf-8 -*-
"""
Vers.1.14, 30.10.2020, Eberhard Waffenschmidt, TH-Köln
...
# Konstanten ###############################################################
Anz_Fragen = 40 # Anzahl Fragen pro Teilnehmer
Anz_Var = 7    # Maximale Anzahl von Frage-Variablen pro Frage
Anz_Res = 1     # Maximale Anzahl von Frage-Ergebnissen pro Frage.
Max_Pkt = 40    # Maximale Anzahl von Punkten im Test
Schema_Note = ["5,0","4,0","3,7","3,3","3,0","2,7","2,3","2,0","1,7","1,3","1,0"]
Schema_Proz = [0,     25,   27,   29,   31,   33,   35,   37,   39,   40,   40]
              # Mindestprozentzahl an Punkten für die korrespondierende Note
Filename_Export = "exp_Ergebnisse_A.xlsx"
Filename_Export_public = 'exp_Ergebnisse_A_pub.xlsx'
Filename_Export_PSSO = 'exp_Ergebnissse_A_psso.xlsx'
IRT_Frame_Name = "irt_frame_A.xlsx"
...
try:
    df1 = pd.read_excel (r'Ergebnisse_A.xlsx', sheet_name='Auswertung für alle Benutzer', header=None)
except xlrd.biffh.XLRDError:
    df = pd.ExcelFile (r'Ergebnisse_A.xlsx')#, header=None)
    sheet_names = df.sheet_names
    df1 = pd.DataFrame()
    for name in sheet_names[1:]:
        df1 = df1.append(df.parse(name, header=None))

print ("ILIAS-Ergebnisse OK.")
# Fragenpool aus Excel-Tabelle einlesen
df2 = pd.read_excel (r'Fragenpool.xlsx', sheet_name='Tabelle1')
...
df3 = pd.DataFrame()#pd.read_excel (r'Bonuspunkte.xlsx', sheet_name='Tabelle1')
print ("Bonuspunkte OK.")
# PSSO-Teilnehmerliste einlesen
df4 = pd.DataFrame()#pd.read_excel (r'PSSO_Teilnehmer.xls', sheet_name='First Sheet', skiprows=3)
...
for Zeile in range(Anz_Zeilen):
   txt = I_D[Zeile,0]
   if I_D[Zeile, 1] == "Bitte ignorieren!":
       continue
   if (txt.__class__ == Dummytext.__class__):
      if txt.startswith(Name_Marker):
...
try:
    ...
    Questiontypes_Pool = df2['question_type']  # "Formelfrage" oder "Freitextfrage"
try:
    Titels_Pool = df2['Question Title']
except KeyError:
    Titels_Pool = df2['question_title'].dropna()
Gleichungen1_Pool = df2['res1_formula'] # Nur Formel 1 wird ausgewertet
Toleranzen1_Pool = df2['res1_tol']
Punkte1_Pool = df2['res1_points']
